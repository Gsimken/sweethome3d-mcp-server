...
The plugin artifact is created at `target/sh3d-mcp-plugin-1.3.0.sh3p`.
...
```bash
# Linux / macOS
cp target/sh3d-mcp-plugin-1.3.0.sh3p ~/.sweethome3d/plugins/

# Windows (Git Bash)
cp target/sh3d-mcp-plugin-1.3.0.sh3p "$APPDATA/eTeks/Sweet Home 3D/plugins/"
```
